...
import de.kp.spark.core.io.HBaseReader

class HBaseSource(@transient sc:SparkContext) extends Serializable {
 
  def connect(config:Configuration,req:ServiceRequest,names:List[String],types:List[String]):RDD[Map[String,Any]] = {
    
    val columnfamily = req.data("columnfamily")
